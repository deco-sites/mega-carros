...
const setup = ({ rootId, behavior, interval }: Props) => {
  ...
  for (let pv = 0; pv < (prev?.length ?? 0); pv++) {
    prev?.item(pv).addEventListener("click", onClickPrev);
  }

  for (let nx = 0; nx < (prev?.length ?? 0); nx++) {
    next?.item(nx).addEventListener("click", onClickNext);
  }
  ...
  return () => {
    ...
    for (let pv = 0; pv < (prev?.length ?? 0); pv++) {
      prev?.item(pv).removeEventListener("click", onClickPrev);
    }

    for (let nx = 0; nx < (prev?.length ?? 0); nx++) {
      next?.item(nx).removeEventListener("click", onClickNext);
    }
